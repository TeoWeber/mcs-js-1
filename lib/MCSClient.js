...
const SetFloorMessage = require('./messages/setFloor');
const SetSubtitlesMessage = require('./messages/setSubtitles');
...
class MCSClient extends MCSBaseClient {
  ...
  setLayout (roomId, layoutId) {
    ...
    const message = new SetLayoutMessage(roomId, layoutId);
   ...
   * @param {Number} userId
   */
  setFloor (roomId, floorId) {
    if (!roomId || typeof(roomId) !== 'string') {
      throw new Error('Invalid roomId');
    }

    if (typeof(floorId) !== 'number') {
      throw new Error('Invalid floorId');
    }

    const message = new SetFloorMessage(roomId, floorId);
  ...
  /**
   * @param {String} roomId
   * @param {Boolean} subtStatus
   */
  setSubtitles (roomId, subtStatus) {
    if (!roomId || typeof(roomId) !== 'string') {
      throw new Error('Invalid roomId');
    }

    if (typeof(subtStatus) !== 'boolean') {
      throw new Error('Invalid subtStatus');
    }

    const message = new SetSubtitlesMessage(roomId, subtStatus);
